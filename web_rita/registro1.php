...
echo "<script>var redirecciona = false;</script>";
     ...
     /*
		...
		if ((isset($correo) && !empty($correo)) && (isset($nombre) && !empty($nombre)) && (isset($clave) && !empty($clave)) && (isset($reclave) && !empty($reclave)) && (isset($fdn) && !empty($fdn))) {
			if ($clave == $reclave) {
                ...
                while($rowSexo = mysqli_fetch_assoc($querySexo)){ 
                    ...
                    //echo "Hombre";
					    ...
					    while ($rowRecienRegistrado = mysqli_fetch_assoc($queryRecienRegistrado)){
					        ...
					        $mensaje = "Bienvenid@ a LOS CUADROS DE RITA<br> Para validar tu cuenta pulsa <a href='https://loscuadrosderita.com/demo/validar.php?idUsuario=".$rowRecienRegistrado['id_usuario']."' target='blank'>aquí</a>";
              ...
              include 'sweets/exitoRegistro.html';
...
<section class="sectionRegistro rCompraN" style="">
